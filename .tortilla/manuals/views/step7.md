...
@@ -88,6 +89,7 @@
 ┊88┊89┊
 ┊89┊90┊    this.send = this.send.bind(this);
 ┊90┊91┊    this.onRefresh = this.onRefresh.bind(this);
+┊  ┊92┊    this.renderItem = this.renderItem.bind(this);
 ┊91┊93┊  }
 ┊92┊94┊
...
@@ -141,7 +143,6 @@
 ┊141┊143┊  send(text) {
 ┊142┊144┊    this.props.createMessage({
 ┊143┊145┊      groupId: this.props.navigation.state.params.groupId,
-┊144┊   ┊      userId: 1, // faking the user for now
 ┊145┊146┊      text,
 ┊146┊147┊    }).then(() => {
 ┊147┊148┊      this.flatList.scrollToEnd({ animated: true });
```
```diff
@@ -153,7 +154,7 @@
 ┊153┊154┊  renderItem = ({ item: message }) => (
 ┊154┊155┊    <Message
 ┊155┊156┊      color={this.state.usernameColors[message.from.username]}
-┊156┊   ┊      isCurrentUser={message.from.id === 1} // for now until we implement auth
+┊   ┊157┊      isCurrentUser={message.from.id === this.props.auth.id}
 ┊157┊158┊      message={message}
 ┊158┊159┊    />
 ┊159┊160┊  )
```
```diff
@@ -193,6 +194,10 @@
 ┊193┊194┊}
 ┊194┊195┊
 ┊195┊196┊Messages.propTypes = {
+┊   ┊197┊  auth: PropTypes.shape({
+┊   ┊198┊    id: PropTypes.number,
+┊   ┊199┊    username: PropTypes.string,
+┊   ┊200┊  }),
 ┊196┊201┊  createMessage: PropTypes.func,
 ┊197┊202┊  navigation: PropTypes.shape({
 ┊198┊203┊    navigate: PropTypes.func,
```
```diff
@@ -251,10 +256,10 @@
 ┊251┊256┊});
 ┊252┊257┊
 ┊253┊258┊const createMessageMutation = graphql(CREATE_MESSAGE_MUTATION, {
-┊254┊   ┊  props: ({ mutate }) => ({
-┊255┊   ┊    createMessage: ({ text, userId, groupId }) =>
+┊   ┊259┊  props: ({ ownProps, mutate }) => ({
+┊   ┊260┊    createMessage: ({ text, groupId }) =>
 ┊256┊261┊      mutate({
-┊257┊   ┊        variables: { text, userId, groupId },
+┊   ┊262┊        variables: { text, groupId },
 ┊258┊263┊        optimisticResponse: {
 ┊259┊264┊          __typename: 'Mutation',
 ┊260┊265┊          createMessage: {
```
```diff
@@ -264,8 +269,8 @@
 ┊264┊269┊            createdAt: new Date().toISOString(), // the time is now!
 ┊265┊270┊            from: {
 ┊266┊271┊              __typename: 'User',
-┊267┊   ┊              id: 1, // still faking the user
-┊268┊   ┊              username: 'Justyn.Kautzer', // still faking the user
+┊   ┊272┊              id: ownProps.auth.id,
+┊   ┊273┊              username: ownProps.auth.username,
 ┊269┊274┊            },
 ┊270┊275┊            to: {
 ┊271┊276┊              __typename: 'Group',
```
```diff
@@ -307,7 +312,12 @@
 ┊307┊312┊  }),
 ┊308┊313┊});
 ┊309┊314┊
+┊   ┊315┊const mapStateToProps = ({ auth }) => ({
+┊   ┊316┊  auth,
+┊   ┊317┊});
+┊   ┊318┊
 ┊310┊319┊export default compose(
+┊   ┊320┊  connect(mapStateToProps),
 ┊311┊321┊  groupQuery,
 ┊312┊322┊  createMessageMutation,
 ┊313┊323┊)(Messages);
```
...
```diff
@@ -15,6 +15,7 @@
 ┊15┊15┊import { graphql, compose } from 'react-apollo';
 ┊16┊16┊import { NavigationActions } from 'react-navigation';
 ┊17┊17┊import update from 'immutability-helper';
+┊  ┊18┊import { connect } from 'react-redux';
 ┊18┊19┊
 ┊19┊20┊import { USER_QUERY } from '../graphql/user.query';
 ┊20┊21┊import CREATE_GROUP_MUTATION from '../graphql/create-group.mutation';
```
```diff
@@ -156,7 +157,6 @@
 ┊156┊157┊
 ┊157┊158┊    createGroup({
 ┊158┊159┊      name: this.state.name,
-┊159┊   ┊      userId: 1, // fake user for now
 ┊160┊160┊      userIds: _.map(this.state.selected, 'id'),
 ┊161┊161┊    }).then((res) => {
 ┊162┊162┊      this.props.navigation.dispatch(goToNewGroup(res.data.createGroup));
```
```diff
@@ -235,13 +235,13 @@
 ┊235┊235┊};
 ┊236┊236┊
 ┊237┊237┊const createGroupMutation = graphql(CREATE_GROUP_MUTATION, {
-┊238┊   ┊  props: ({ mutate }) => ({
-┊239┊   ┊    createGroup: ({ name, userIds, userId }) =>
+┊   ┊238┊  props: ({ ownProps, mutate }) => ({
+┊   ┊239┊    createGroup: ({ name, userIds }) =>
 ┊240┊240┊      mutate({
-┊241┊   ┊        variables: { name, userIds, userId },
+┊   ┊241┊        variables: { name, userIds },
 ┊242┊242┊        update: (store, { data: { createGroup } }) => {
 ┊243┊243┊          // Read the data from our cache for this query.
-┊244┊   ┊          const data = store.readQuery({ query: USER_QUERY, variables: { id: userId } });
+┊   ┊244┊          const data = store.readQuery({ query: USER_QUERY, variables: { id: ownProps.auth.id } });
 ┊245┊245┊
 ┊246┊246┊          if (isDuplicateGroup(createGroup, data.user.groups)) {
 ┊247┊247┊            return;
```
```diff
@@ -253,7 +253,7 @@
 ┊253┊253┊          // Write our data back to the cache.
 ┊254┊254┊          store.writeQuery({
 ┊255┊255┊            query: USER_QUERY,
-┊256┊   ┊            variables: { id: userId },
+┊   ┊256┊            variables: { id: ownProps.auth.id },
 ┊257┊257┊            data,
 ┊258┊258┊          });
 ┊259┊259┊        },
```
...
@@ -272,7 +272,12 @@
 ...
 ┊273┊273┊});
 ┊274┊274┊
+┊   ┊275┊const mapStateToProps = ({ auth }) => ({
+┊   ┊276┊  auth,
+┊   ┊277┊});
+┊   ┊278┊
 ┊275┊279┊export default compose(
+┊   ┊280┊  connect(mapStateToProps),
 ┊276┊281┊  userQuery,
 ┊277┊282┊  createGroupMutation,
 ┊278┊283┊)(FinalizeGroup);
```
...
@@ -125,8 +126,7 @@
 ┊125┊126┊  leaveGroup() {
 ┊126┊127┊    this.props.leaveGroup({
 ┊127┊128┊      id: this.props.navigation.state.params.id,
-┊128┊   ┊      userId: 1,
-┊129┊   ┊    }) // fake user for now
+┊   ┊129┊    })
 ┊130┊130┊      .then(() => {
 ┊131┊131┊        this.props.navigation.dispatch(resetAction);
 ┊132┊132┊      })
```
```diff
@@ -231,7 +231,7 @@
 ┊231┊231┊        variables: { id },
 ┊232┊232┊        update: (store, { data: { deleteGroup } }) => {
 ┊233┊233┊          // Read the data from our cache for this query.
-┊234┊   ┊          const data = store.readQuery({ query: USER_QUERY, variables: { id: 1 } }); // fake for now
+┊   ┊234┊          const data = store.readQuery({ query: USER_QUERY, variables: { id: ownProps.auth.id } });
 ┊235┊235┊
 ┊236┊236┊          // Add our message from the mutation to the end.
 ┊237┊237┊          data.user.groups = data.user.groups.filter(g => deleteGroup.id !== g.id);
```
```diff
@@ -239,7 +239,7 @@
 ┊239┊239┊          // Write our data back to the cache.
 ┊240┊240┊          store.writeQuery({
 ┊241┊241┊            query: USER_QUERY,
-┊242┊   ┊            variables: { id: 1 }, // fake for now
+┊   ┊242┊            variables: { id: ownProps.auth.id },
 ┊243┊243┊            data,
 ┊244┊244┊          });
 ┊245┊245┊        },
```
...
@@ -249,12 +249,12 @@
 ┊249┊249┊
 ┊250┊250┊const leaveGroupMutation = graphql(LEAVE_GROUP_MUTATION, {
 ┊251┊251┊  props: ({ ownProps, mutate }) => ({
-┊252┊   ┊    leaveGroup: ({ id, userId }) =>
+┊   ┊252┊    leaveGroup: ({ id }) =>
 ┊253┊253┊      mutate({
-┊254┊   ┊        variables: { id, userId },
+┊   ┊254┊        variables: { id },
 ┊255┊255┊        update: (store, { data: { leaveGroup } }) => {
 ┊256┊256┊          // Read the data from our cache for this query.
-┊257┊   ┊          const data = store.readQuery({ query: USER_QUERY, variables: { id: 1 } }); // fake for now
+┊   ┊257┊          const data = store.readQuery({ query: USER_QUERY, variables: { id: ownProps.auth.id } });
 ┊258┊258┊
 ┊259┊259┊          // Add our message from the mutation to the end.
 ┊260┊260┊          data.user.groups = data.user.groups.filter(g => leaveGroup.id !== g.id);
```
```diff
@@ -262,7 +262,7 @@
 ┊262┊262┊          // Write our data back to the cache.
 ┊263┊263┊          store.writeQuery({
 ┊264┊264┊            query: USER_QUERY,
-┊265┊   ┊            variables: { id: 1 }, // fake for now
+┊   ┊265┊            variables: { id: ownProps.auth.id },
 ┊266┊266┊            data,
 ┊267┊267┊          });
 ┊268┊268┊        },
```
...
@@ -270,7 +270,12 @@
 ...
 ┊271┊271┊});
 ┊272┊272┊
+┊   ┊273┊const mapStateToProps = ({ auth }) => ({
+┊   ┊274┊  auth,
+┊   ┊275┊});
+┊   ┊276┊
 ┊273┊277┊export default compose(
+┊   ┊278┊  connect(mapStateToProps),
 ┊274┊279┊  groupQuery,
 ┊275┊280┊  deleteGroupMutation,
 ┊276┊281┊  leaveGroupMutation,
```

##### Changed client&#x2F;src&#x2F;screens&#x2F;new-group.screen.js
```diff
@@ -14,6 +14,7 @@
 ┊14┊14┊import AlphabetListView from 'react-native-alphabetlistview';
 ┊15┊15┊import update from 'immutability-helper';
 ┊16┊16┊import Icon from 'react-native-vector-icons/FontAwesome';
+┊  ┊17┊import { connect } from 'react-redux';
 ┊17┊18┊
 ┊18┊19┊import SelectedUserList from '../components/selected-user-list.component';
 ┊19┊20┊import USER_QUERY from '../graphql/user.query';
```
```diff
@@ -314,12 +315,17 @@
 ┊314┊315┊};
 ┊315┊316┊
 ┊316┊317┊const userQuery = graphql(USER_QUERY, {
-┊317┊   ┊  options: (ownProps) => ({ variables: { id: 1 } }), // fake for now
+┊   ┊318┊  options: ownProps => ({ variables: { id: ownProps.auth.id } }),
 ┊318┊319┊  props: ({ data: { loading, user } }) => ({
 ┊319┊320┊    loading, user,
 ┊320┊321┊  }),
 ┊321┊322┊});
 ┊322┊323┊
+┊   ┊324┊const mapStateToProps = ({ auth }) => ({
+┊   ┊325┊  auth,
+┊   ┊326┊});
+┊   ┊327┊
 ┊323┊328┊export default compose(
+┊   ┊329┊  connect(mapStateToProps),
 ┊324┊330┊  userQuery,
 ┊325┊331┊)(NewGroup);
```
